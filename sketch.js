...
let axes = [["之", "的"], ["說", "曰"], ["不", "了"], ["我", "他"], ["pc1", "pc2"]];
let active_axis;
...
function setup() {
    ...
    active_max_x = get_max_val(active_axis[0], datapoints)
    active_max_y = get_max_val(active_axis[1], datapoints)
    active_min_x = get_min_val(active_axis[0], datapoints)
    active_min_y = get_min_val(active_axis[1], datapoints)
...
function draw() {
    ...
    let use_loadings;
    if (state == 0 || state == 1) {
        use_points = datapoints
        use_loadings = text_loadings;
    } else if (state == 2) {
    ...
    max_x = get_max_val(active_axis[0], use_points)
    max_y = get_max_val(active_axis[1], use_points)
    min_x = get_min_val(active_axis[0], use_points)
    min_y = get_min_val(active_axis[1], use_points)
